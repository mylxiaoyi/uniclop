...
**/
void corner_detect(const CImg<uint8_t>& image, const int barrier, std::vector<FASTFeature>& corners);
...
*/
void nonmax(const CImg<uint8_t>& image,  const int barrier, std::vector<FASTFeature>& corners, std::vector<FASTFeature>& nonmax_corners);
