...
/** 
* The BTLocalMatcherT class is a subclass of LocalMatcherT and implements the Birchfield-Tomasi 
* method for intensity matching.
*
* @author Abhijit Ogale
*/
template<typename T>
class BTLocalMatcherT : public LocalMatcherT<T>
{
public:

  BTLocalMatcherT(); /**< Default constructor with no parameters */
  virtual ~BTLocalMatcherT(); /**< Destructor */
  virtual bool setImagePair(const ImageT<T> & i1, const ImageT<T> & i2);
  virtual bool setParams(int nparams, double*params);
  virtual const ImageT<double> getMatch(int shiftx, int shifty);
  virtual const ImageT<double> getRawMatch(int shiftx, int shifty);

protected:
 ImageT<double> mImage1; /**< copy of input image 1. */
  ImageT<double> mImage2; /**< copy of input image 2 */

  ImageT<double> mMin1; /**< precomputed minimum image from i1 for speeding up Birchfield Tomasi matching. */
  ImageT<double> mMax1; /**< precomputed maximum image from i1 for speeding up Birchfield Tomasi matching. */
  ImageT<double> mMin2; /**< precomputed minimum image from i2 for speeding up Birchfield Tomasi matching. */
  ImageT<double> mMax2; /**< precomputed maximum image from i2 for speeding up Birchfield Tomasi matching. */
...
*/
template<typename T>
bool BTLocalMatcherT<T>::setImagePair(const ImageT<T> & i1, const ImageT<T> & i2)
{
...
template<typename T>
const ImageT<double> BTLocalMatcherT<T>::getMatch(int shiftx, int shifty=0)
{
  double tempAlpha;
...
*/
template<typename T>
const ImageT<double> BTLocalMatcherT<T>::getRawMatch(int shiftx, int shifty=0)
{
  int height, width, nchannels;
  int i1,j1,k,i2,j2,jlo,jhi,ilo,ihi;
  double matchvalue1, matchvalue2, temp;

  ImageT<double> result(mImage1,false);
