...
void testStereo(char*imgfilename1, char*imgfilename2, double minshift, double maxshift)
{
  ...
  if(img1 && img2 && imgLeftDisparity && imgRightDisparity && imgLeftOcclusions && imgRightOcclusions)
  {
    ...
    stereoManager.doStereoMatching(*ovaImg1, *ovaImg2, minshift, maxshift, 
				   *ovaLeftDisp, *ovaRightDisp, *ovaLeftOcc, *ovaRightOcc);

    //DISPLAY RESULTS

    //BEGIN: rescale disparity maps to range (0,1) so that they can be displayed by OpenCV
    ImageT<double> ovtleftDisp, ovtrightDisp;
    ...
    if(false)
      { // just for debugging
	int dimx, dimy, dimv;
	ovaImg1->getSize(dimx, dimy, dimv);
	cout << "ovaImg1 size == " << dimx << ", " << dimy << ", " << dimv<< endl;
	cout << "ovaImg1(50,50,0) == " << ovaImg1->getPixel(50,50,0) << endl; 
	cout << "ovaImg1(50,50,1) == " << ovaImg1->getPixel(50,50,1) << endl; 
	cout << "ovaImg1(50,50,2) == " << ovaImg1->getPixel(50,50,2) << endl; 
	cout << "ovaLeftDisp(50,50) == " << ovaLeftDisp->getPixel(50,50,0) << endl; 
	cout << "ovaaLeftOcc(50,50) == " << ovaLeftOcc->getPixel(50,50,0) << endl; 
      }
...
void testOpticalFlow(char*imgfilename1, char*imgfilename2, double minshiftX, double maxshiftX, double minshiftY, double maxshiftY)
{
  ...
  if(img1 && img2 && imgU1 && imgV1 && imgO1 && imgU2 && imgV2 && imgO2)
  {
    ...
    FlowDiffuseMatcherT<double> flowDiffuseMatcher;

    //create general optical flow algorithm execution manager instance
    FlowT<double> flowManager;
    ...
    ImageT<double> ovtU1, ovtV1, ovtU2, ovtV2;
