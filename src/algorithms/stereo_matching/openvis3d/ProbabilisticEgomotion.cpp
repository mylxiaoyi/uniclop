...
#include <cstdlib> 
#include <iomanip>
#include "ProbabilisticEgomotion.hpp"
...
ImageT<float> getProbDist(ImageT<float> phases1, ImageT<float> phases2, int i0, int j0, int lb_i, int lb_j, int ub_i, int ub_j)
{
  int pad = 20;
  ...
  ImageT<float> result = ImageT<float>(ub_i-lb_i, ub_j-lb_j, 1);
...
ProbabilityDistributions getProbDists(ImageT<float> phases1, ImageT<float> phases2, int numpoints)
{
  int maxrez    = 300;
  float minprob = 1/(float)maxrez;

  ImageT<float> g     = ImageT<float>(numpoints,maxrez,1);
  ImageT<float> pt1_x = ImageT<float>(numpoints,1,1);
  ImageT<float> pt1_y = ImageT<float>(numpoints,1,1);
  ImageT<float> pt2_x = ImageT<float>(numpoints,maxrez,1);
  ImageT<float> pt2_y = ImageT<float>(numpoints,maxrez,1);
  ...
  for(int n=0; n<numpoints; n++){
    ...
    ImageT<float> dist = getProbDist(phases1, phases2, i0, j0, lb_i, lb_j, ub_i, ub_j);
...
MotionParameters compute3Dmotion(ImageAdapter & image1, ImageAdapter & image2, float f, float pp_x, float pp_y, int numpoints, int numsearches)
{
  ...
  ImageT<float> im1Phases = im1.getGaborPhaseStack();
  ImageT<float> im2Phases = im2.getGaborPhaseStack();
