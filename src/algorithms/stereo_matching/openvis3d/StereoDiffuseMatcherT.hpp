...
/**
* The StereoDiffuseMatcherT implements the fast diffusion based stereo matching discussed in
...
*/
template<typename T>
class StereoDiffuseMatcherT : public StereoGlobalMatcherT<T>
{
public:

  StereoDiffuseMatcherT(); /**< Default constructor with no parameters */

  virtual ~StereoDiffuseMatcherT(); /**< Destructor */
  ...
  */
  virtual bool doMatching(ImageT<T> & i1, ImageT<T> & i2, double minshift, double maxshift, ImageT<double> & leftDisparityMap, ImageT<double> & rightDisparityMap, ImageT<double> & leftOcclusions, ImageT<double> & rightOcclusions);
...
public:
  ImageT<double>  bestprobabL, bestprobabR; // <<< rodrigo's dirty hacks
  ImageT<double>  confidenceL, confidenceR; // <<< rodrigo's dirty hacks

protected:
  LocalMatcherT<T> * mLocalMatcher;

  virtual void computeGoodnessHoriz (const ImageT<double> & iMatch, const ImageT<double> & iConductivity, ImageT<double> & iGoodness);
  virtual void computeGoodnessVert  (const ImageT<double> & iMatch, const ImageT<double> & iConductivity, ImageT<double> & iGoodness);

};


template<typename T>
StereoDiffuseMatcherT<T>::StereoDiffuseMatcherT()
{
}
...
template<typename T>
bool StereoDiffuseMatcherT<T>::doMatching(ImageT<T> & i1, ImageT<T> & i2, double minshift, double maxshift, ImageT<double> & leftDisparityMap, ImageT<double> & rightDisparityMap, ImageT<double> & leftOcclusions, ImageT<double> & rightOcclusions)
{
  int height, width, channels;
  int i,j,jproj;

  ImageT<double> 
	iMatch, iGoodnessVert, 
	iGoodnessHoriz, iGoodnessL, iGoodnessR, 
	// bestprobabL, bestprobabR, // set as public members
	bestprobabRshifted;
  ImageT<double> iMatchR; // rodrigo's hacks

  ImageT<bool> maskL, maskR;

  i1.getDimensions(height,width,channels);

  // resetDimensions will also fill with zeros the images
  ...
  confidenceL.resetDimensions(height,width);
  confidenceR.resetDimensions(height,width);
  ...
  for(int shift=static_cast<int>(minshift); shift<=maxshift; shift++)
  {
    ...
    ImageT<double> &iMatchL = iMatch;
    iMatchR = shiftImageXY(iMatchL, shift, 0);
    confidenceL.copyMasked(maskL, iMatchL);
    confidenceR.copyMasked(maskR, iMatchR);
  }
...
template<typename T>
bool StereoDiffuseMatcherT<T>::setParams(int nparams, double*params)
{
  return true;
}

template<typename T>
void StereoDiffuseMatcherT<T>::setLocalImageMatcher(LocalMatcherT<T> & localImageMatcher)
{
  mLocalMatcher = &localImageMatcher;
}

template<typename T>
void StereoDiffuseMatcherT<T>::computeGoodnessHoriz(const ImageT<double> & iMatch, const ImageT<double> & iConductivity, ImageT<double> & iGoodness)
{
...
template<typename T>
void StereoDiffuseMatcherT<T>::computeGoodnessVert(const ImageT<double> & iMatch, const ImageT<double> & iConductivity, ImageT<double> & iGoodness)
{
  int i, j, height, width, channels;
